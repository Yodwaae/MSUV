    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, movePoint.position - new Vector3(0, .25f, 0)) <= .02f)//Added offset since movePoint is not at player origins (0,.25,0)
        {
            ...
            if (Mathf.Abs(Input.GetAxisRaw("Horizontal")) == 1f)//Check for X axis input
            {
                ...
                if (!Physics2D.OverlapCircle(movePoint.position + new Vector3(Mathf.Sign(Input.GetAxisRaw("Horizontal")) * .25f, -Mathf.Sign(Input.GetAxisRaw("Horizontal")) * .125f, 0), .2f, obstacles))
                {
            ...
            else if (Mathf.Abs(Input.GetAxisRaw("Vertical")) == 1f)//Check for Y axis input
            {
                ...
                if (!Physics2D.OverlapCircle(movePoint.position + new Vector3(Mathf.Sign(Input.GetAxisRaw("Vertical"))*.25f, Mathf.Sign(Input.GetAxisRaw("Vertical"))*.125f, 0), .2f, obstacles))
                {
